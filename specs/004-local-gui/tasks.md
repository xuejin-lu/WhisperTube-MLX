...
- [ ] T001 Add pinned Gradio dependency in `requirements-ui.txt` and deterministic CI installation in `.github/workflows/test.yml`
- [ ] T002 Create the local GUI module scaffold in `whispertube/gui.py` and test scaffold in `tests/test_gui.py`

## Phase 2: Foundational

- [ ] T003 Define GUI status/result data boundaries and injected pipeline callable types in `whispertube/gui.py`
- [ ] T004 Define explicit local-only Blocks, queue, event, and launch configuration constants in `whispertube/gui.py`
...
- [ ] T005 [US1] Add RED request-validation, one-call propagation, running-state, and stale-clearing tests in `tests/test_gui.py`
- [ ] T006 [US1] Implement framework-independent request handling and running-state adaptation in `whispertube/gui.py`
- [ ] T007 [US1] Add RED Gradio component/event wiring tests for labeled URL input, Transcribe action, visible status, and one-active-event settings in `tests/test_gui.py`
- [ ] T008 [US1] Implement the Gradio Blocks layout and chained running-to-terminal event in `whispertube/gui.py`
...
- [ ] T009 [US2] Add RED success, approved-error, unexpected-error, and sensitive-traceback suppression tests in `tests/test_gui.py`
- [ ] T010 [US2] Implement terminal status and error adaptation while preserving approved pipeline categories in `whispertube/gui.py`
- [ ] T011 [US2] Add RED local-only construction and launch configuration tests for loopback, share, telemetry, monitoring, CORS, file roots, and API exposure in `tests/test_gui.py`
- [ ] T012 [US2] Implement local-only queue and launch behavior plus CLI configuration in `whispertube/gui.py`
...
- [ ] T013 [US3] Add RED transcript-root/type/content validation and byte-equivalent preview/download tests in `tests/test_gui.py`
- [ ] T014 [US3] Implement safe transcript validation, UTF-8 preview loading, and download result adaptation in `whispertube/gui.py`

## Phase 6: Polish & Cross-Cutting Concerns

- [ ] T015 [P] Update `README.md` with v0.4 installation, launch, privacy, and local-use instructions
- [ ] T016 Run focused GUI tests, the full deterministic suite, `compileall`, and `git diff --check`
- [ ] T017 Run the real local browser GUI smoke with the approved public video and small model; inspect transcript/audio artifacts and record any Human Gate
